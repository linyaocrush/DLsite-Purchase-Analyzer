(function(){
  ...
  // ── Settings-aware entry point ──
  const _settings = window.__DLsiteAnalyzerSettings || null;

  ui.injectStyles();

  if (_settings) {
    // Tampermonkey mode: apply settings directly, skip interactive prompts
    (async () => {
      cleanup();
      ui.addLanguageSwitcher();
      ...
      const detailMode = _settings.mode === "detail";
      let dlurl = "https://www.dlsite.com/maniax/mypage/userbuy/=/type/all/start/all/sort/1/order/1/page/";
      if (detailMode) {
        const typeVal = _settings.workType || "0";
        dlurl = typeVal === "0" ? dlurl.replace(/type\/[^/]+/, "type/all") : dlurl.replace(/type\/[^/]+/, `type/${typeVal}`);
      } else {
        dlurl = dlurl.replace(/type\/[^/]+/, "type/all");
      }
      ...
      // Exchange rate
      if (i18n.current !== "ja") {
        if (_settings.exchangeRateSource === "api") {
          await i18n.fetchExchangeRates();
        }
      }
      const currencyConfig = i18n.getCurrencyConfig();
      let exchangeRate = i18n.current === "ja" ? 1 : currencyConfig.rate;
      appState.currencyLang = i18n.current;
      appState.customRates[i18n.current] = exchangeRate;
      ...
      // Apply exclude threshold
      const excludeThreshold = _settings.excludeThreshold || 0;
      result.genreCount = [...result.genreCount.entries()].sort((a, b) => b[1].count - a[1].count);
      result.makerCount = [...result.makerCount.entries()].sort((a, b) => b[1].count - a[1].count);
      const filteredGenreCount = excludeThreshold === 0 ? result.genreCount : result.genreCount.filter(([, entry]) => entry.count >= excludeThreshold);
      const filteredMakerCount = excludeThreshold === 0 ? result.makerCount : result.makerCount.filter(([, entry]) => entry.count >= excludeThreshold);
      ...
      // Apply chart setting
      appState.showCharts = !!_settings.chartDisplay;
      appState.result = result;
      appState.filteredGenreCount = filteredGenreCount;
      appState.filteredMakerCount = filteredMakerCount;
      appState.exchangeRate = exchangeRate;
      ...
      if (appState.showCharts) {
        await charts.loadChartJS();
        if (detailMode) charts.drawGenreChart(filteredGenreCount, result.works, "bar");
        charts.drawMakerChart(filteredMakerCount, result.works, "bar");
        charts.drawTimelineChart(result.works);
        charts.drawCumulativeChart(result.works);
      }
    ...
    // Console mode: original interactive flow
    main();
  }

  // Expose re-run hook for Tampermonkey userscript
  window.__DLsiteRerun = () => {
    if (window.__DLsiteAnalyzerSettings) {
      // Re-inject with same settings
      const s = document.createElement("script");
      s.dataset.dlsiteCore = "true";
      s.src = document.querySelector('script[data-dlsite-core]')?.src || "";
      if (s.src) {
        document.querySelectorAll('script[data-dlsite-core]').forEach(el => el.remove());
