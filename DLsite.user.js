...
(function(){
  ...
  const SETTINGS_KEY = "dlsiteAnalyzerUserSettings";
  ...
  const userSettings = {
    load() {
      const saved = gmStorage.get(SETTINGS_KEY, {});
      return {
        language: saved.language || null,
        useDefaults: Boolean(saved.useDefaults),
        defaults: saved.defaults || {}
      };
    },
    save(settings) {
      gmStorage.set(SETTINGS_KEY, settings);
    }
  };
  const utils = {
  ...
  const initialSettings = userSettings.load();
  if (initialSettings.language) {
    localStorage.setItem("dlsiteAnalyzerLang", initialSettings.language);
  }
  i18n.init();
  ...
  const DISABLE_LANGUAGE_SWITCHER = true;
  ...
  const ui = {
    ...
    addLanguageSwitcher() {
      if (DISABLE_LANGUAGE_SWITCHER) return;
  ...
  const main = async (options = {}) => {
    const { useDefaults = false, defaults = {} } = options;
    cleanup();
    ...
    const getValue = async (key, promptFn) => {
      if (useDefaults && defaults && defaults[key] !== undefined && defaults[key] !== null && defaults[key] !== "") {
        return defaults[key];
      }
    ...
    let detailMode = true;
    const quickView = await getValue("quickView", () => modal.customChoice(i18n.t("quickViewPrompt"), [
      { label: i18n.t("yes"), value: "y" },
      { label: i18n.t("no"), value: "n" }
    ]));
    if (quickView && quickView.toString().toLowerCase() === "y") detailMode = false;
    ...
    if (detailMode) {
      ...
      const typeChoice = await getValue("typeChoice", () => modal.customChoice(i18n.t("selectTypePrompt"), typeOptionsArr));
    ...
    appState.currencyLang = i18n.current;
    if (useDefaults && defaults.exchangeRate) {
      const presetRate = parseFloat(defaults.exchangeRate);
      if (!isNaN(presetRate) && presetRate > 0) {
        exchangeRate = presetRate;
      }
    } else if (i18n.current !== "ja") {
    ...
    console.groupEnd();
    const excludeResponse = await getValue("excludeThreshold", () => modal.customPrompt(i18n.t("excludePrompt"), "0"));
    ...
    const showChart = await getValue("showCharts", () => modal.customChoice(i18n.t("showChartPrompt"), [
  ...
  const openSettingsPanel = () => {
    const prefs = userSettings.load();
    const langInput = prompt("设置语言 (zh/en/ja)：", prefs.language || i18n.current);
    const language = ["zh", "en", "ja"].includes((langInput || "").trim()) ? langInput.trim() : prefs.language || i18n.current;
    const useDefaults = confirm("是否启用默认设置并在运行时可直接使用？");

    const defaults = prefs.defaults || {};
    const quickView = prompt("默认是否开启快速查看？(y/n)", defaults.quickView || "n");
    const typeChoice = prompt("默认作品类型代码 (0=全部, 12=同人:所有, 2=同人:全年龄, 1=同人:男性向, 3=同人:女性向, 13=商业:所有, 9=商业:全年龄, 4=商业:男性向, 14=漫画:所有, 10=漫画:全年龄, 7=漫画:男性向, 11=漫画:女性向)", defaults.typeChoice || "0");
    const exchangeRateStr = prompt("默认汇率（数字，日元->当前货币）", defaults.exchangeRate != null ? defaults.exchangeRate : "");
    const exchangeRate = exchangeRateStr && !isNaN(parseFloat(exchangeRateStr)) ? parseFloat(exchangeRateStr) : defaults.exchangeRate;
    const excludeThresholdStr = prompt("默认最少作品数过滤值（整数）", defaults.excludeThreshold != null ? defaults.excludeThreshold : "0");
    const excludeThreshold = excludeThresholdStr && !isNaN(parseInt(excludeThresholdStr, 10)) ? parseInt(excludeThresholdStr, 10) : defaults.excludeThreshold || 0;
    const showCharts = prompt("默认是否显示图表？(y/n)", defaults.showCharts || "y");
  ...
  const startAnalyzer = async (forceDefaults = false) => {
    const prefs = userSettings.load();
    if (prefs.language) {
      i18n.setLanguage(prefs.language);
      localStorage.setItem("dlsiteAnalyzerLang", prefs.language);
    }
    ui.injectStyles();
    const hasDefaults = prefs.defaults && Object.keys(prefs.defaults).length > 0;
    let shouldUseDefaults = forceDefaults || (prefs.useDefaults && hasDefaults);
    if (!forceDefaults && prefs.useDefaults && hasDefaults) {
      shouldUseDefaults = confirm("检测到默认设置，是否直接使用默认配置运行脚本？");
    }
    await main({ useDefaults: shouldUseDefaults, defaults: prefs.defaults || {} });
  };

  const registerMenu = () => {
    if (typeof GM_registerMenuCommand === "function") {
      GM_registerMenuCommand("启动 DLsite Analyzer", () => startAnalyzer(false));
      GM_registerMenuCommand("使用默认设置启动", () => startAnalyzer(true));
      GM_registerMenuCommand("设置", () => openSettingsPanel());
    } else {
      console.warn("GM_registerMenuCommand 不可用，默认直接运行脚本。");
      startAnalyzer(false);
    }
  };
