...
[Frankfurter API](https://frankfurter.dev/)（欧州中央銀行データ）から JPY → CNY / USD のリアルタイムレートを取得し、24時間キャッシュ。API が利用できない場合は内蔵デフォルト値にフォールバック。実行時にモーダルでカスタムレートを設定でき、言語ごとに独立して記憶されます。

| 言語 | 通貨 | データソース |
|:----:|:----:|:------------:|
| 简体中文 | CNY | Frankfurter API リアルタイム |
| English | USD | Frankfurter API リアルタイム |
| 日本語 | JPY | 変換なし |
